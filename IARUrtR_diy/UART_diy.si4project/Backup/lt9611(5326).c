...
{
    printk("liuyuelin: %s",__FUNCTION__);
	int rc = 0;
	struct i2c_msg msg;
	u8 buf[2] = { reg, val };
...
static int lt9611_check_hpd(void *client, u32 flags)
{
    printk("liuyuelin: %s",__FUNCTION__);
  struct lt9611 *pdata =
		lt9611_get_platform_data(client);
	u8 reg_val = 0;
	int connected = 0
...
static int lt9611_hpd_Interrupt_Handle(struct lt9611 *pdata)
{
	...
	connected = lt9611_check_hpd(client, flags);
...
static irqreturn_t lt9611_irq_thread_handler(int irq, struct lt9611 *pdata)
{
	...
	lt9611_handle_cec_intr(pdata, hdmi_cec_status);
